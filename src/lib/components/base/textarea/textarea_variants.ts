...
export const textareaVariants = tv({
	base: 'w-full rounded-md border border-border bg-surface px-2.5 py-2 font-sans text-(length:--text-md) leading-normal text-foreground outline-none transition-[border-color,box-shadow] duration-120 ease-[ease] resize-vertical placeholder:text-foreground-subtle hover:border-border-strong disabled:pointer-events-none disabled:cursor-not-allowed disabled:bg-surface-2 disabled:text-foreground-subtle disabled:opacity-70 focus-visible:border-ring focus-visible:shadow-[0_0_0_3px_color-mix(in_oklch,var(--ring)_22%,transparent)]',
	variants: {
		state: {
			default: '',
